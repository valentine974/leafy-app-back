...
router.post("/create-user", (req, res, next) => { // ADD THE MIDDLE WARE and verif for creating profile only for HRs
  ...
  User.findOne({ email })
    ...
    .then((createdUser) => {
      ...
      const { email, name, _id, isNewEmployee } = createdUser;

      // Create a new object that doesn't expose the password
      const user = { email, name, _id, isNewEmployee };
  ...
  User.findOne({ email })
    .then((foundUser) => {
      ...
      if (passwordCorrect ) {
        ...
        const { _id, email, name, isNewEmployee } = foundUser;

        // Create an object that will be set as the token payload
        const payload = { _id, email, name, isNewEmployee }; 
